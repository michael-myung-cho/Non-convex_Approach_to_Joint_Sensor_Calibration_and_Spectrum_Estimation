...
for ii=1:10
        ...
        try
            if nlevel==0 % Call cvx for trace minimization
                ...
                cvx_begin sdp quiet
                    ...
                    variable u(N) complex
                    variable NN(N,N) complex hermitian
                    TT=toeplitz(u);
                    TT=0.5*(TT+TT'); % taking care of non-symmetric warning
                minimize lambda * square_pos(norm(XX-Y_noisy_cov_rec.*HH,'fro')) + real(trace(TT))
                subject to
                    TT - XX >= 0;
                    XX == Y_noisy_cov_rec.*HH-NN;
                    HH >= 0;
                    XX >= 0;
                    HH(2,1) == 100;
                cvx_end
        ...
        try
            if nlevel==0 % Call cvx for trace minimization
                ...
                for itrRPANM=1:10
                    ...
                    TT=toeplitz(u);
                    TT=0.5*(TT+TT'); % taking care of non-symmetric warning
                    minimize lambda * square_pos(norm(XX-Y_noisy_cov_rec.*HH,'fro')) + trace(W*TT)
                    subject to
                    TT - XX >= 0;
                    XX == Y_noisy_cov_rec.*HH-NN;
                    HH >= 0;
    ...
    resBuf = [resBuf;N,sparsity,RL,nlevel,L,nSuccAlge,nSuccOptWF,nSuccAltFW,nSuccPANM,nSuccRPANM];
